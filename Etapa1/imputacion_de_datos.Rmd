...
Momentaneamente tomaremos el método de imputación LOFC y luego elegiremos el mejor.

```{r}
write.csv(dataset_CE_LOFC, "DATASET_CENTRO_LIMPIO")

write.csv(dataset_NTE_LOFC, "DATASET_NORTE_LIMPIO")
```

```{r}
write.csv(dataset_CE_arima, "DATASET_CENTRO_LIMPIO2")
```


```{r}
dataset_NTE_arima <- na_kalman(ts_NTE, model = "auto.arima")

write.csv(dataset_NTE_arima, "DATASET_NORTE_LIMPIO2")
```